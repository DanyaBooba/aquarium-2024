...
<form action="{{ route('settings.profile.email.store') }}" onsubmit="sendForm('{{ route('settings') }}')" method="post">
    ...
    <div>
        ...
        <input type="email" name="currentEmail" class="form-control" id="currentEmail"  placeholder="{{ __('Почта') }}" onInput="data()" required>
    </div>
    <div class="input-group input-password" id="password-form1">
        <div>
            <label for="currentPassword">{{ __('Пароль') }}</label>
    ...
    <div>
        ...
        <input type="email" name="newEmail" class="form-control" id="newEmail"  placeholder="{{ __('Почта') }}" onInput="data()" required>
        <p>
